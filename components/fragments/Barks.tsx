...
const Barks = () => {
  ...
  const fetchBarks = async ({ pageParam = "" }) => {
    const res = await fetch(`/api/barks?cursor=${pageParam}`);
    return res.json();
  };

  const { data, isLoading, hasNextPage, fetchNextPage, isFetchingNextPage } =
    useInfiniteQuery<InfiniteBarks>({
      queryKey: ["barks"],
      queryFn: fetchBarks,
    ...
    return (
      <div className="grow flex justify-center items-center">
        <div role="status">
          <svg
            aria-hidden="true"
            className="inline w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-indigo-600"
            viewBox="0 0 100 101"
            fill="none"
...
const Bark = ({ bark }: { bark: barksWithUser }) => {
  return (
